  ...
  */
  ...
  function start($login="", $password="")  {
    ...
    if (strlen($this->login) > 0 && strlen($this->password) > 0) {
      $linkidentifier = mysqli_connect($this->hostname, $this->login, $this->password) ;
      if (!$linkidentifier) {
				$this->setError("<b>Database Connect Error</b> : Couldn't connect to the database Wrong login and password or Wrong host name") ;
			} else {
      ...
      $linkidentifier = mysqli_connect($this->hostname, $this->login) ;
      if ($linkidentifier) {
        $this->id = $linkidentifier ;
        if (strlen($this->db)>0) {
          if (!mysqli_select_db($this->id, $this->db)) {
    ...
    if ($this->getUseCluster()) {
            ...
            $wlinkidentifier = mysqli_connect($this->whostname, $this->wlogin, $this->wpassword) ;
            if ($wlinkidentifier) {
                $this->wid = $wlinkidentifier ;
                if (strlen($this->db)>0) {
                if (!mysqli_select_db($this->wid, $this->db)){
                ...
                $wlinkidentifier = mysqli_connect($this->whostname, $this->wlogin) ;
                if ($wlinkidentifier) {
                    $this->wid = $wlinkidentifier ;
                    if (strlen($this->db)>0) {
                    if (!mysqli_select_db($this->wid, $this->db)) {
