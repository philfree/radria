...
//    }
  ...
  //  }
    ...
    if (preg_match("/^select/i", $this->sql_query)) { 
		...
		$rquery = mysqli_query($this->query_connexion, $this->sql_query);
    ...
    if (!$this->max_rows) {
      $this->num_rows = @mysqli_num_rows($rquery) ;
    }
  ...
  function getInsertId($table="", $field="") {
    if (!($this->insert_id > 0)) {
        $this->insert_id = mysqli_insert_id($this->query_connexion) ;
    }
  ...
  function fetch($result = 0) {
    if ($result>0) {
      ...
      $rowobject = mysqli_fetch_object($this->result) ;
    }
    $this->cursor++ ;
    $this->data = $rowobject ;
  return $rowobject ;
   ...
   */
    function escapeString($value) {
        if (!function_exists("mysqli_real_escape_string")) {
						$conx  = $this->dbCon->id;
            function mysqli_real_escape_string( $conx , $value) {
        ...
        return mysqli_real_escape_string($this->dbCon->id, $value);
    }
    /**
     * quote
